...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!parsed.success) {
    console.error('[admin/save-polygons] validation failed:', parsed.error.format());
  ...
  if (!token || !owner || !repo) {
    console.error('[admin/save-polygons] Missing GitHub env');
    return Response.json({ error: 'GitHub integration not configured' }, { status: 500 });
  }

  const path = 'src/data/floor-plans.json';
  ...
  try {
    ...
    if (Array.isArray(current.data) || current.data.type !== 'file') {
      return Response.json({ error: 'floor-plans.json not found in repo' }, { status: 404 });
    }
    ...
    const commit = await octokit.rest.repos.createOrUpdateFileContents({
      ...
      repo,
      path,
      message: 'admin: update floor-plan polygons',
      content: contentB64,
    ...
    console.log(
      `[admin/save-polygons] committed sha=${commit.data.commit.sha?.slice(0, 7)}`,
    );
    return Response.json(
      { ok: true, commitSha: commit.data.commit.sha },
      { status: 200 },
    );
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Unknown';
    console.error('[admin/save-polygons] GitHub API error:', msg);
